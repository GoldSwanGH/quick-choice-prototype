...
class TestExampleTestCase2(unittest.TestCase):
    def setUp(self):
    ...
    def test_t_ordering_output(self):
        # Проверка корректности вывода метода t_ordering
        decision_model = DecisionModel(self.criteria_list, self.alternatives_df, self.preferences_list)
        ...
        print(result_df)
        # Сравнение DataFrame
        pd.testing.assert_frame_equal(result_df.loc[["Alternative A"]], expected_df)
